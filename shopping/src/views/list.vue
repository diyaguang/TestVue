    ...
    <div v-show="list.length">
        <div class="list-control">
            <div class="list-control-filter">
                <span>品牌：</span>
                <span class="list-control-filter-item" :class="{on:item === filterBrand}" v-for="item in brands" @click="handleFilterBrand(item)" :key="item.id">{{ item }}</span>
            </div>
            <div class="list-control-filter">
                <span>颜色：</span>
                <span class="list-control-filter-item" :class="{on:item === filterColor}" v-for="item in colors" @click="handleFilterColor(item)" :key="item.id">{{ item }}</span>
            </div>
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                order: '',  //排序依据，sales（销量），cost-desc（价格降序），cost-asc（价格升序）
                filterBrand: '',
                filterColor: ''
            }
        },
        ...
        computed:{
            ...
            filteredAndOrderedList(){
                let list = [...this.list];    //展开运算符，相当于克隆一份数据
                if(this.filterBrand !== ''){
